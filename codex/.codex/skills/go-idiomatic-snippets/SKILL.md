---
name: go-idiomatic-snippets
description: Provide idiomatic Go patterns and ready-to-adapt snippets for writing, refactoring, and reviewing Go code. Use when requests mention idiomatic Go, clean Go style, best-practice snippets, or examples for errors, context, interfaces, concurrency, HTTP handlers, repositories, and testing.
---
...
!`rg -N --no-messages '^go ' -g 'go.mod' . | awk '{print $2}' | sort | uniq -c | sort -nr | awk 'NR==1{print $2; exit}' || echo unknown`
